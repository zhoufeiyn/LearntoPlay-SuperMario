...
# Training Function
# -----------------------------
def train_video_model(resume_from=None):
    """训练视频生成模型"""
    ...
    # 如果指定了继续训练的路径，加载参数
    if resume_from:
        model_path = os.path.join(cfg.out_dir, resume_from)
        if os.path.exists(model_path):
            model.load_state_dict(torch.load(model_path, map_location=device))
            optimizer = torch.optim.AdamW(model.parameters(), lr=cfg.lr*0.8, weight_decay=0.01)
            print(f"✅ 从 {model_path} 继续训练，学习率: {cfg.lr*0.8}")
        else:
            print(f"❌ 模型文件不存在: {model_path}")
            print(f"🚀 从头开始训练，学习率: {cfg.lr}")
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "train":
            if len(sys.argv) > 2:
                ...
                print(f"🚀 从 {model_file} 继续训练视频生成模型")
                train_video_model(resume_from=model_file)
            else:
                # 从头开始训练
                print("🚀 开始训练视频生成模型")
                train_video_model()
            ...
            print("❌ 未知参数")
            print("💡 使用方法:")
            print("   python flow_matching_mario_video.py train                    # 从头训练模型")
            print("   python flow_matching_mario_video.py train video_model.pt   # 从指定模型继续训练")
